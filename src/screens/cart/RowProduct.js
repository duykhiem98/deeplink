...
class RowProduct extends PureComponent {
  render() {
    const { rowData, onClickItem, decrease, increase, onDelete } = this.props;
      ...
      >
        ...
        <View style={styles.right}>
          <View style={styles.viewRow}>
            <View style={{ flex: 1 }}>
              <Text style={styles.itemTitle} numberOfLines={1}>{rowData.product.name}</Text>
              <View style={styles.viewVariant}>
                <Text style={styles.itemVariant}>{variantTxt}</Text>
              </View>
            </View>
            <MaterialIcons name='clear' size={20} color={Colors.txtGrey} onPress={() => onDelete(rowData)} />
          </View>
          ...
          <View style={[styles.viewRow, { marginTop: 10 }]}>
            ...
            <View style={styles.viewRow}>
              <TouchableOpacity style={styles.viewSub} onPress={() => decrease(rowData)}>
                <MaterialIcons name='remove' size={20} color={Colors.white} />
              </TouchableOpacity>
              <Text style={styles.txtCount}>{rowData.quantity > rowData.product.quantity ? rowData.product.quantity : rowData.quantity}</Text>
              <TouchableOpacity style={styles.viewAdd} onPress={() => increase(rowData)}>
